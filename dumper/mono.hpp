...
namespace dumper
{
	...
	auto setup_exports( ) -> void
	{
		...
		runtime_invoke = ( mono_runtime_invoke ) GetProcAddress( mod, "mono_runtime_invoke" );
	}
	...
	auto dump_methods( const char* _namespace, const char* class_name ) -> void 
	{
		void* iterator = NULL;
		const auto mono_class = class_from_name( base_image, _namespace, class_name );

		while ( auto method = class_get_methods( mono_class, &iterator ) )
		{
			printf( "[~] dumped %s\n", method_full_name( method, true ) );
		}
	}
